...
def _tx(amount, tx_id="d1", category="Groceries"):
    return {"id": tx_id, "amount": amount, "category": category, "date": "2026-06-01", "item": "Dinner"}
...
def test_create_claim_from_debit(client, fake_supabase):
    ...
    claims_tbl.insert.return_value.execute.return_value.data = [{
        "id": "c1", "debit_tx_id": "d1", "total": 100, "my_share": 25,
        "expected": 75, "category": "Groceries", "status": "open",
    }]
    _set_table_router(fake_supabase, {"transactions": tx_tbl, "claims": claims_tbl})

    resp = client.post("/api/claims", json={"debit_tx_id": "d1", "my_share": 25, "counterparty": "Friend"})
...
def test_link_credit(client, fake_supabase):
    ...
    resp = client.post("/api/claims/c1/credits", json={"credit_tx_id": "cr1", "allocated_amount": 80})
...
def test_link_rejects_negative_tx(client, fake_supabase):
    ...
    resp = client.post("/api/claims/c1/credits", json={"credit_tx_id": "cr1", "allocated_amount": 5})
...
def test_link_rejects_unknown_credit(client, fake_supabase):
    ...
    resp = client.post("/api/claims/c1/credits", json={"credit_tx_id": "missing", "allocated_amount": 5})
...
def test_link_rejects_over_allocation(client, fake_supabase):
    ...
    resp = client.post("/api/claims/c1/credits", json={"credit_tx_id": "cr1", "allocated_amount": 60})
...
def test_unlink_credit(client, fake_supabase):
    links_tbl = MagicMock()
    links_tbl.delete.return_value.eq.return_value.eq.return_value.execute.return_value.data = [{"id": "l1"}]
...
def test_settle_and_reopen(client, fake_supabase):
    ...
    claims_tbl.update.return_value.eq.return_value.execute.return_value.data = [{"id": "c1", "status": "settled"}]
