...
def _validated_profile(payload):
    ...
    if salary < 0 or payday not in range(1, 32):
        raise HTTPException(
            status_code=422, detail="Salary must be non-negative and payday must be 1?31."
        )
...
def _validated_goal(payload):
    ...
    if not name or target_amount <= 0 or not 0 <= allocation_percentage <= 100:
        raise HTTPException(
            status_code=422, detail="Enter a name, positive target, and 0?100% allocation."
        )
    return {
        "name": name,
...
def sync_automatic_contributions(db: Client = Depends(get_db)):
    ...
    contributions = (
        db.table("saving_contributions").select("goal_id, contribution_month, kind").execute().data
    )
    existing = {
        (row["goal_id"], row["contribution_month"])
            ...
            db.table("saving_contributions").insert(
                {
                    "goal_id": goal["id"],
                    "amount": round(
                        float(profile["salary"]) * float(goal["allocation_percentage"]) / 100, 2
                    ),
                    "kind": "automatic",
