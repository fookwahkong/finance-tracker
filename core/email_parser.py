...
def parse(body: str) -> dict:
    if _RECEIVED_AMOUNT_RE.search(body):
        return _parse(body, _RECEIVED_AMOUNT_RE, _RECEIVED_DATE_RE, _RECEIVED_MERCHANT_RE,
                      fmt="received", direction="in", date_has_year=True)
    if _GIRO_AMOUNT_RE.search(body):
        return _parse(body, _GIRO_AMOUNT_RE, _GIRO_DATE_RE, _GIRO_MERCHANT_RE,
                      fmt="giro", direction="out", date_has_year=False)
    return _parse(body, _AMOUNT_RE, _DATE_RE, _MERCHANT_RE,
                  fmt="transfer", direction="out", date_has_year=False)


def _parse(body, amount_re, date_re, merchant_re, fmt: str, direction: str, date_has_year: bool) -> dict:
